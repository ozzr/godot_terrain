Godot Engine - Terrain Module v1.1.2 Documentation
===============================

**Table of content**
- [Godot Engine - Terrain Module v1.1.2 Documentation](#godot-engine---terrain-module-v112-documentation)
	- [Overview](#overview)
	...
	- [The Terrain Editor](#the-terrain-editor)
		...
		- [Dedicated Editors](#dedicated-editors)
			...
			- [Path Painter](#path-painter)
				- [Properties](#properties-3)
	- [TerrainPath](#terrainpath)
		- [The Node](#the-node)
		- [The TerrainPath Editor and Gizmos](#the-terrainpath-editor-and-gizmos)
...
6. ISSUE: BiomeItem - fix Item [Min Scale is not considered in game](https://github.com/ozzr/godot_terrain/issues/14)
...
The `Collision Group` has some useful properties:
...
* `TERRAIN_BASE_HEIGHTMAP` exists to store the deafult heights for the terrain and height data for `paths` and other separate modifications that will be mixed together in the `TERRAIN_HEIGHTMAP` which is the one that should be used in shaders
...
* `TERRAIN_SHADOWS_HEIGHTMAP` it is used to draw distant shadows casted by the terrain and big meshes onto the surface. You can think of it as a heightmap that stores not the surface height, but the hight at which is the shadow casted down onto the surface. If an object height is below the shadow height then that object is shaded.
...
* **Albedo Bump**: Expects a texture where `RGB channles` contains the ALBEDO information and the `alpha channel` the heights information. At the moment of writing, the module doesn't provide an in house method to do this but there are plenty of tool out there to combine image channels into a new file.
...
The `Biome` class is used as a storage class for `BiomeItems` which are the ones displayed by the `BiomeRenderer`. Beyond this purpose, the class doesn't do much more. Biomes are accessed using their unique index which is in the range of `0-254` so the max amount of biomes that can be stored is `255`. The major performance penalties for any game will come from rendering the biomes, so taking good care of how we configure each `BiomeItem` is mandatory.
...
***
<img src="images/classes/biome_item/014-terrain_data_biomes_items_properties_top_half.png" align="right">
...
* **Distribution Type**: Defines the method used to generate the default sample of points. It can be any of:  `RANDOM`,  `JITTERED GRID` or  `POISSON` (default).
...
* **Draw Distance**: `1.0` by default. Relative max distance at which the item can be rendered. Depends on the values specified at the `BiomeRenderer`  
...
* **Distance Fraction**: `1.0` by default. Relative value used to calculate the max draw distance at which the `lod` will be visible withing the max `Draw Distance` range.
...
<img src="images/classes/biome_item/015-terrain_data_biomes_items_properties_bottom_half.png" align="right">
...
Internally the terrain is rendered in patches (1 patch = 1 draw call), with each patch having its predefined and prebaked quadtree which is traversed every frame to extract the visible chunks or nodes. The fact that it is prebaked gives a nice performance gain since we dont have to regenerate the entire tree every time. The rendering is directly managed using the `VisualServer` API and multimeshes.

##### Properties

The visible properties varies depending on which state the renderer is.
...
* **Min Distance**: `250` by default. Distance to camera at which the lowest lod (`0`) will be rendered. The value depends on the `LOD Count` and will increase automatically to the lowest aceptable value at which the LOD morphing wont break. 
...
The default `BiomeRenderer` is the `GridRenderer` and the only of this type right now. It uses a tile based rendering approach where for every tile it reads the `biomemap` indentifies which biomes to consider and loops over every item starting from those of the highest type like trees and down to decals sorted by their `footprint`.
...
* **Decal/Grass/Shrubs/Objects/Trees Distance**: Relative distances to the `Max Render Distance` beyond which no defined item type will be rendered.
...
* **Create Navigation Mesh**: This will add a NavigationMeshInstance child with a `NavigationMesh` created from the combined patches of the terrain as to ensure patch continuity
![Baked NavigationMeshInstance](images/019.1-terrain_menu-generated_navmesh.png)
> **IMPORTANT**: Currently no biome mesh is taken into account when generating the `NavigationMesh`. 

> **NOTE:** The generated **`MeshInstance`** (hidden by default ) is a decimated geometry of the terrain and it is the source for the NavigationMesh. The simplification strength is selected after pressing the `Create Navigation Mesh` option. If you dont have any use for this mesh, **REMOVE IT**

> **WARNING**: Baking a NavigationMesh with this **HUGE** mesh might crush the editor. Experiment changing some `NavigationMesh`  parameters until you get something that works for you  

* **Create Mesh From Terrain**: Exports each patch of the Terrain as a separate `MeshInstance` decimated as stated in a pregeneration process.
  ![Generated Terrain Meshes](images/019.2-terrain_menu-generated_meshes.png)

  > **IMPORTANT**: The generation depends on the current Terrain resolution value. 

  > **NOTE**: Normals Smoothing can be activated in a pregeneration step along with the decimation value. 
...
<img src="images/editors/022-tools_bar.png" align="right">
...
* **Path paint**: Sets the editor to add paths to the terrain surface. Paths can be mainly of two type: `Roads like paths` that rise the terrain, or River like paths which lower the surface height. Upon seleting this tool, the `editor brush` will adapt to it.
...
<img src="images/editors/023-brush_editors.png" align="right">
...
* **Preserve Paths**: When editing the terrain, if this property is checked, the editor will preserve the path data stored in the green channel of the `TERRAIN_BASE_HEIGHTMAP` texture, else it will destroy it.
 
***
#### Paint Textures Editor

<img src="images/editors/024-paint_textures.png" align="right">
...
<img src="images/editors/025-biome_manager.png" align="right" width =200 height=200>
...
![Erosion and Filters ](images/editors/026-eroders_and_filters.png)

***
#### Path Painter

The Path Painter is intended to be used to modify the path to be painted properties. It has two basic modes: 
1. **Paint Road Like Paths** 
2. **Paint River Like Paths**

The main difference between modes is that paths exported in the river mode will have its height right at the origial curve height, unlike road paths where the height id offsetted by the `Height Offset` property
> HINT: Each path will  store the blend value used to mix the path height with the terrain height in the green channel of the TERRAIN_BASE_HEIGHTMAP texture
> 
##### Properties
* **Export Path**: Will generate a TerrainPath Node from the generated curve. Instanced as a child of the current terrain being edited
* **Width**: This determines the main width of the path. Useful for roads, but less important for rivers where the blend distance would have better results
* **Blend Distance**: This is the distance along which the terrain height will blend with the path height.
* **Height Offsete**: Used to control the height at which the path center should be carved. This can be possitve or negative for roads, but can only have negative values for rivers.
* **Max Slope**: To control the maximun degree of freedom tolerated by the path in the y axis when painting.
* **Min Step Size**: Minimum distance between control points in the path.
* **Blending Curve**: Used to blend between the terrain height and the path height. By default will display as a smoothstep interpolation.
* **Tessellation Properties**: To control the curve points density when painting.

![Path Painter](images/editors/027-path_editor.png)
...
### The Node
![Alt text](images/classes/terrain_path/terrain_path.png)

The `TerrainPath` class inherits from the `GeometryInstance` class and it is intended to be used directly for creating and modifying the the internal mesh from the suplied `Curve3D`.

By default the newly instanced `TerrainPath` class will have its geometry setted to `NONE` but there are thre different shapes to be chosed from: `PLANE`, `BOX`, `CYLINDER`. **Planes** are better suited for **Rivers** while Boxes are better for roads. **Cylinders**... they are useful.

![Alt text](images/classes/terrain_path/terrain_path-properties.png)

Properties prefixed with `geometry` will affect the generated 
mesh only, while those prefixed with `path` will work on the Curve3D and the internally generated path.

> **HINT:** Paths baked by the TerrainPath are different than those obtained from Curve3D.get_baked_points

Additionally other groups of properties might be visible depending on the geometry type selected.

> **HINT:** in shader the COLOR.a goes from 0 to 1 and describes the length of the path
> 
> **HINT:** When using a Plane shape, the COLOR.rgb channels store the direction from the current vertex to the next center vertex of the path

### The TerrainPath Editor and Gizmos

![Alt text](images/classes/terrain_path/terrain_path-gizmo_no_tilt.png)

The Editor is based on the original PathEditorPlugin implementation from the engine but extended with new features and tweaked for better usability. As you can see above, there are new gizmos attatched for better management of the Curve3D data.

`Orange handles` are in charge of controlling the width of the path. With values can be previewed also in the Node inspector. Visually, the with at a given control point can be previewed b looking at the red lines that crosses the position handles. `Green handles` are in charge of modifying the tilt. If the tilt is set to be used a circle will appear around the control points with the tilt handle at the end of the green line (normal direction) `(see image below)`
 
![Alt text](images/classes/terrain_path/terrain_path-gizmo_w_tilt.png)

#### Editor Menu
The editor menu offers the same options as those available when editing a Path node, but since we are working with TerrainPath nodes now there are other properties to play with.

![Alt text](images/classes/terrain_path/terrain_path-editor_menu.png)

* **Mirror Closed Endpoints**: Will treat endpoint control handles as one when both path endpoints matches 
* **Create Trimesh Static Body**: Will generate a collision body for the path geometry if a valid internal mesh exists.
* **Export Mesh Sibling**: Will generate a MeshInstance node with the internal mesh.
* **Unwrap UV2**: Will prepare the mesh for lightmap usage.
* **Invert Path**: Inverts the direction of the path by inverting the order of the points in the base Curve3D.
* **Node Origin to Center**: Changes the control point coordinates and centers the TerrainPath Node transform to the center of the path.
* **Bake Path into Terrain**: Popups a second windows with properties similar to those in the  `PathPainter`  (see below image), searches for every Terrain node available in the escene and bakes the path into the surface.
  
![Alt text](images/classes/terrain_path/terrain_path-menu_bake.png)
